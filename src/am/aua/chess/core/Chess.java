...
public class Chess implements Cloneable {
    ...
    public boolean isKingUnderAttack(PieceColor kingColor) {
        ...
        positions = getAllDestinationsByColor(opponentColor);

        for (int i = 0; i < positions.size(); i++)
            if (positions.get(i).equals(kingPosition))
                return true;
        ...
        for (int i = 0; i < BOARD_RANKS; i++)
            for (int j = 0; j < BOARD_FILES; j++)
                if (board[i][j] != null && board[i][j].getPieceColor() == color) {
                    ArrayList<Position> current = board[i][j].allDestinations(this, Position.generateFromRankAndFile(i, j));
                    ...
                    for (int k = 0; k < current.size(); k++) {
                        ...
                        Position[] posarr = new Position[current.size()]; // creating same size array
                        posarr = current.toArray(posarr);// put list items to array
                        result = Position.appendPositionsToArray(result, posarr); // calling method with parameter ArrayList result and Array postwar
                    }
                }
